...
// 2. Configure CORS (kept minimal as per your setup)
...
async function setupDatabase() {
  try {
    ...
    // Keep your original order; non-fatal if emrs exists already
...
// 4. --- Health (handy for smoke tests) ---
...
app.get('/api/emrs', async (req, res) => {
  ...
  try {
    ...
    // Hide patient_password in responses
...
// Generate PDF (tight top spacing + clean signature)
app.get('/api/emrs/:id/generate-pdf', async (req, res) => {
  try {
    ...
    // Medical info
    doc.fontSize(12).font('Helvetica-Bold').text('Symptoms:').font('Helvetica').text(emr.symptoms || 'N/A').moveDown(0.5);
    doc.fontSize(12).font('Helvetica-Bold').text('Medical History:').font('Helvetica').text(emr.medical_history || 'N/A').moveDown(0.5);
    doc.fontSize(12).font('Helvetica-Bold').text('Current Medication:').font('Helvetica').text(emr.current_medication || 'N/A').moveDown(0.75);

    // Documents
    const mdocs = safeArray(emr.medical_documents);
    if (mdocs.length > 0) {
      doc.fontSize(12).font('Helvetica-Bold').text('Medical Documents:').font('Helvetica').moveDown(0.25);
      mdocs.forEach(d => {
        doc.text(`- ${d.name || 'N/A'}: ${d.url || 'N/A'} (Password: ${d.password || 'N/A'})`).moveDown(0.1);
      });
      doc.moveDown(0.75);
    } else {
      doc.fontSize(12).font('Helvetica-Bold').text('Medical Documents:').font('Helvetica').text('N/A').moveDown(0.75);
    }

    // Physician report
    doc.fontSize(14).text("Physician's Report", { underline: true }).moveDown(0.5);
    doc.fontSize(12).font('Helvetica-Bold').text('Diagnosis:').font('Helvetica').text(emr.doctor_diagnosis || 'N/A').moveDown(0.5);
    doc.fontSize(12).font('Helvetica-Bold').text('Report & Findings:').font('Helvetica').text(emr.doctor_report || 'N/A').moveDown(0.5);
    doc.fontSize(12).font('Helvetica-Bold').text('Recommendations:').font('Helvetica').text(emr.doctor_recommendations || 'N/A').moveDown(0.5);
